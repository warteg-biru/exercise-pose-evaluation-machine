...
class PushUpInitialPoseDetector():
    def __init__(self):
        self.MODEL_PATH\
            = "/home/kevin/projects/exercise_pose_evaluation_machine/models/push_up_initial_pose_model/push_up_initial_pose_model.h5"
...
def get_files_and_labels():
    pos_dir = "/home/kevin/projects/push_up_init_pose_data/pos"
    neg_dir = "/home/kevin/projects/push_up_init_pose_data/neg"

    # ambil minimum length, supaya pos dan neg data jumlah nya sama
    pos_files_len = len(os.listdir(pos_dir))
    neg_files_len = len(os.listdir(neg_dir))
    min_len = pos_files_len if pos_files_len < neg_files_len else neg_files_len

    # X - files
    pos_files = [
        os.path.join(pos_dir, filename) for filename in os.listdir(pos_dir)[:min_len]
    ]
    neg_files = [
        os.path.join(neg_dir, filename) for filename in os.listdir(neg_dir)[:min_len]
    ]
    # y - labels
...
def test(MODEL_PATH, kp_extractor):
    ...
    test_pos_dir = pos_dir = "/home/kevin/projects/push_up_init_pose_data/test_data/pos"
    test_neg_dir = pos_dir = "/home/kevin/projects/push_up_init_pose_data/test_data/neg"
    test_pos_files = [
